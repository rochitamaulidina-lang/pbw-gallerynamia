...
<head>
    ...
    <title>Daftar Pembelian - Gallery Namia</title>
        ...
        <div id="content" class="p-3 p-md-4">
            ...
            <div class="card-table">
                <div class="card-header-custom">
                    <h5><i class="fas fa-file-alt mr-2" style="color:#ff8c1a;"></i> Daftar Pembelian</h5>
                    <a href="{{ route('pembelian.create') }}" class="btn btn-warning-custom">
                        <i class="fas fa-plus-circle mr-1"></i> Tambah Pembelian
                    </a>
                </div>
                ...
                <div class="table-wrapper">
                    <table class="data-table">
                        <thead>
                            <tr>
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'no_beli', 'order' => ($sortColumn == 'no_beli' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        NO BELI
                                        <span class="sort-icon">
                                            @if($sortColumn == 'no_beli') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                        </span>
                                    </a>
                                </th>
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'no_faktur', 'order' => ($sortColumn == 'no_faktur' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        NO FAKTUR
                                        <span class="sort-icon">
                                            @if($sortColumn == 'no_faktur') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                        </span>
                                    </a>
                                </th>
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'tgl_beli', 'order' => ($sortColumn == 'tgl_beli' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        TANGGAL
                                        <span class="sort-icon">
                                            @if($sortColumn == 'tgl_beli') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                        </span>
                                    </a>
                                </th>
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'no_supplier', 'order' => ($sortColumn == 'no_supplier' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        SUPPLIER
                                        <span class="sort-icon">
                                            @if($sortColumn == 'no_supplier') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                        </span>
                                    </a>
                                </th>
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'no_pegawai', 'order' => ($sortColumn == 'no_pegawai' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        PEGAWAI
                                        <span class="sort-icon">
                                ...
                                <th>
                                    <a href="{{ route('pembelian.index', ['sort' => 'total_beli', 'order' => ($sortColumn == 'total_beli' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                        TOTAL
                                        <span class="sort-icon">
                                            @if($sortColumn == 'total_beli') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                        </span>
                                    </a>
                                </th>
                                <th>FAKTUR</th>
                        ...
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td>
                                    @if($p->faktur_file)
                                    <a href="{{ asset('uploads/faktur/'.$p->faktur_file) }}" target="_blank" class="btn btn-sm-action" style="background:#28a745; color:white;">
                                        <i class="fa-regular fa-file-image"></i> Lihat
                                    </a>
                                    @else
                                ...
                                <td style="white-space:nowrap;">
                                    <a class="btn btn-sm-action btn-detail" href="{{ route('pembelian.show', $p->no_beli) }}">
                                        <i class="fa-solid fa-eye"></i> Detail
                                    </a>
                                </td>
                                <td>
                                    <a class="btn btn-sm-action btn-edit" href="{{ route('pembelian.edit', $p->no_beli) }}">
                                        <i class="fa-solid fa-pen-to-square"></i> Ubah
                                    </a>
                                    <button class="btn btn-sm-action btn-delete" onclick="showDeleteModal('{{ $p->no_beli }}', 'pembelian')">
                            ...
                            @empty
                            <tr>
                                <td colspan="8" style="text-align:center; padding:20px;">Tidak ada data pembelian</td>
                            </tr>
                            @endforelse
                        </tbody>
                        <tfoot>
                            <tr class="total-row">
                                <td colspan="5" class="total-label">
                                    <strong>
                                        ...
                                        <span style="margin-left: 5px;">TOTAL SEMUA PEMBELIAN</span>
                                    </strong>
                                </td>
                                <td class="total-amount">
                                    <strong>
                                        Rp {{ number_format($pembelian->sum('total_beli'), 0, ',', '.') }}
                                    </strong>
                                </td>
                                <td colspan="2"></td>
                            </tr>
                ...
                <div class="pagination">
                    <div class="pagination-info">
                        Showing {{ $pembelian->firstItem() ?? 0 }} to {{ $pembelian->lastItem() ?? 0 }} of {{ $pembelian->total() }} entries
                    </div>
                    <div class="pagination-buttons">
                        @if ($pembelian->onFirstPage())
                        <span class="page-link disabled">Previous</span>
                        @else
                        <a class="page-link" href="{{ $pembelian->previousPageUrl() }}">Previous</a>
                        @endif
                        ...
                        <a class="page-link" href="{{ $pembelian->nextPageUrl() }}">Next</a>
                        @else
