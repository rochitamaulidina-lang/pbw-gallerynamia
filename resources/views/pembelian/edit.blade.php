...
<head>
    ...
    <title>Ubah Pembelian - Gallery Namia</title>
    ...
    <style>
        /* ========== CSS SAMA PERSIS DENGAN HALAMAN BARANG ========== */
    ...
    <div id="content" class="p-3 p-md-4">
        ...
        <div class="form-card">
            <div class="card-header-custom">
                <h5><i class="fas fa-edit mr-2" style="color:#ff8c1a;"></i> Ubah Pembelian</h5>
            </div>
            <div class="card-body-custom">
                <form action="{{ route('pembelian.update', $pembelian->no_beli) }}" method="POST" enctype="multipart/form-data">
                    @csrf
                    @method('PUT')

                    <div class="form-group-custom">
                        <label><i class="fas fa-hashtag"></i> NO BELI</label>
                        <input type="text" class="form-control-custom" name="no_beli" value="{{ $pembelian->no_beli }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-tag"></i> NO FAKTUR</label>
                        <input type="text" name="no_faktur" class="form-control-custom" value="{{ old('no_faktur', $pembelian->no_faktur) }}">
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-calendar-alt"></i> TANGGAL BELI</label>
                        <input type="date" name="tgl_beli" class="form-control-custom" value="{{ old('tgl_beli', $pembelian->tgl_beli) }}" required>
                    </div>
                    ...
                    <div class="form-group-custom">
                        ...
                        <select class="form-control-custom" name="no_pegawai" required>
                            ...
                            <option value="{{ $p->no_pegawai }}" {{ old('no_pegawai', $pembelian->no_pegawai) == $p->no_pegawai ? 'selected' : '' }}>
                                {{ $p->nama_pegawai }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group-custom">
                        ...
                        <select class="form-control-custom" name="no_supplier" required>
                            <option value="">--- Pilih Supplier ---</option>
                            @foreach($supplier as $s)
                            <option value="{{ $s->no_supplier }}" {{ old('no_supplier', $pembelian->no_supplier) == $s->no_supplier ? 'selected' : '' }}>
                                {{ $s->nama_supplier }}
                            </option>
                            @endforeach
                        </select>
                    ...
                    <div class="form-group-custom">
                        <label><i class="fas fa-image"></i> Upload Faktur (Gambar)</label>
                        <input type="file" name="faktur_file" class="form-control-custom" accept="image/jpeg,image/png,image/jpg">
                        ...
                        <div class="faktur-preview">
                            <img src="{{ asset('uploads/faktur/'.$pembelian->faktur_file) }}" alt="Faktur">
                            <a href="{{ asset('uploads/faktur/'.$pembelian->faktur_file) }}" target="_blank" class="btn btn-sm-action" style="background:#17a2b8; padding:4px 12px; border-radius:30px; color:white; text-decoration:none;">
                    ...
                    <div class="form-group-custom">
                        <label><i class="fas fa-money-bill-wave"></i> TOTAL BELI</label>
                        <input type="text" class="form-control-custom" value="Rp {{ number_format($pembelian->total_beli, 0, ',', '.') }}" readonly>
                    </div>
                    ...
                    <div class="mt-3" style="display: flex; gap: 10px; justify-content: flex-end;">
                        <a href="{{ route('pembelian.index') }}" class="btn btn-action btn-cancel">
