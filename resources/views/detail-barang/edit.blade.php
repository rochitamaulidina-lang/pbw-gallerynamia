...
<head>
    ...
    <title>Ubah Detail Barang - Gallery Namia</title>
    ...
    <style>
        ...
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .btn-update { background: linear-gradient(95deg, #ffb347, #ff8c1a); color: white; box-shadow: 0 4px 12px rgba(255, 140, 26, 0.3); }
        .btn-update:hover { transform: translateY(-2px); box-shadow: 0 6px 16px rgba(255, 140, 26, 0.4); color: white; }
        ...
        .stok-info {
            margin-top: 8px; padding: 6px 10px; border-radius: 12px; font-size: 0.7rem; display: flex; align-items: center; gap: 8px;
        }
    ...
    <div id="content" class="p-3 p-md-4">
        ...
        <div class="form-card">
            <div class="card-header-custom">
                <h3><i class="fas fa-edit"></i> Ubah Detail Barang</h3>
            </div>
            <div class="card-body-custom">
                ...
                <form action="{{ route('detail-barang.update', $barang->no_barang . '-' . $detail->no_bahan) }}" method="POST">
                    @csrf
                    @method('PUT')
                    ...
                    <div class="form-group-custom">
                        ...
                        <input type="text" class="form-control-custom" value="Rp {{ number_format($detail->bahanBaku->harga_beli ?? 0, 0, ',', '.') }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-calculator"></i> QTY PAKAI</label>
                        <input type="number" name="qty_pakai" id="qtyPakai" class="form-control-custom" value="{{ $detail->qty_pakai }}" required min="1">
                        <div id="stokInfoContainer"></div>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-chart-line"></i> SUBTOTAL BOM</label>
                        <input type="text" id="subtotalBom" class="form-control-custom" readonly value="Rp {{ number_format($detail->subtotal_bom, 0, ',', '.') }}">
                    </div>

                    <div class="info-badge">
                        <i class="fas fa-info-circle"></i> Subtotal BOM akan dihitung ulang otomatis jika Qty Pakai diubah.
                    </div>
...
<script>
    $(document).ready(function() {
        ...
        var hargaBeli = {{ $detail->bahanBaku->harga_beli ?? 0 }};
        var stokTersedia = {{ $detail->bahanBaku->stok_bahan ?? 0 }};
        var currentQty = {{ $detail->qty_pakai }};
        var satuan = '{{ $detail->bahanBaku->satuan ?? '' }}';

        function updateStokInfo() {
            var qty = parseInt($('#qtyPakai').val()) || 0;
            var stok = stokTersedia + currentQty;

            if (qty > stok) {
                $('#stokInfoContainer').html(`
                    <div class="stok-info danger">
                        <i class="fas fa-times-circle"></i>
                        <span><strong>Peringatan!</strong> Qty Pakai (${qty.toLocaleString('id-ID')}) melebihi stok tersedia (${stok.toLocaleString('id-ID')} ${satuan})!</span>
                    </div>
                `);
                ...
                $('#stokInfoContainer').html(`
                    <div class="stok-info danger">
                        ...
                        <span>Qty Pakai harus lebih dari 0!</span>
                    </div>
                `);
                $('#submitBtn').prop('disabled', true);
            } else {
                ...
                $('#stokInfoContainer').html(`
                    <div class="stok-info success">
                        ...
                        <span>Stok tersedia: ${stok.toLocaleString('id-ID')} ${satuan} | Sisa stok: ${sisaStok.toLocaleString('id-ID')} ${satuan}</span>
                    </div>
                `);
        ...
        $('#qtyPakai').on('input', updateStokInfo);
        updateStokInfo();
    });
