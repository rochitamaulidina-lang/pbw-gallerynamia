...
<head>
    ...
    <title>Daftar Detail Barang - Gallery Namia</title>
    ...
    <style>
        /* ========== CSS SAMA PERSIS DENGAN INDEX BARANG ========== */
    ...
    <div id="content" class="p-3 p-md-4">
        ...
        <div class="card-table">
            <div class="card-header-custom">
                <h5><i class="fas fa-cubes mr-2" style="color:#ff8c1a;"></i> Daftar Detail Barang (BOM)</h5>
                <a href="{{ route('detail-barang.create') }}" class="btn btn-warning-custom">
                    <i class="fas fa-plus-circle mr-1"></i> Tambah Detail Barang
                </a>
            </div>
            ...
            <div class="table-wrapper">
                <table class="data-table">
                    <thead>
                        <tr>
                            <th>
                                <a href="{{ route('detail-barang.index', ['sort' => 'no_barang', 'order' => ($sortColumn == 'no_barang' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                    NO BARANG
                                    <span class="sort-icon">
                                        @if($sortColumn == 'no_barang') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                    </span>
                                </a>
                            </th>
                            <th>
                                <a href="{{ route('detail-barang.index', ['sort' => 'no_bahan', 'order' => ($sortColumn == 'no_bahan' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                    NO BAHAN
                                    <span class="sort-icon">
                            ...
                            <th>
                                <a href="{{ route('detail-barang.index', ['sort' => 'qty_pakai', 'order' => ($sortColumn == 'qty_pakai' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                    QTY PAKAI
                                    <span class="sort-icon">
                                        @if($sortColumn == 'qty_pakai') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                    </span>
                                </a>
                            </th>
                            <th>
                                <a href="{{ route('detail-barang.index', ['sort' => 'subtotal_bom', 'order' => ($sortColumn == 'subtotal_bom' && $sortOrder == 'DESC') ? 'ASC' : 'DESC']) }}">
                                    SUBTOTAL BOM
                                    <span class="sort-icon">
                                        @if($sortColumn == 'subtotal_bom') {!! $sortOrder == 'DESC' ? '↓' : '↑' !!} @else ↕ @endif
                                    </span>
                                </a>
                            </th>
                            <th>AKSI</th>
                    ...
                    <tbody>
                        ...
                        <tr>
                            <td>{{ $d->barang->no_barang ?? '-' }} - {{ $d->barang->nama_barang ?? '-' }}</td>
                            <td>{{ $d->bahanBaku->no_bahan ?? '-' }} - {{ $d->bahanBaku->nama_bahan ?? '-' }}</td>
                            <td>{{ $d->qty_pakai }}</td>
                            <td>Rp {{ number_format($d->subtotal_bom, 0, ',', '.') }}</td>
                            <td style="white-space:nowrap;">
                                <a class="btn btn-sm-action btn-edit" href="{{ route('detail-barang.edit', $d->no_barang . '-' . $d->no_bahan) }}">
                        ...
                        @empty
                        <tr>
                            <td colspan="5" style="text-align:center; padding:20px;">Tidak ada data detail barang</td>
                        </tr>
                        @endforelse
                    </tbody>
                </table>
            ...
            <div class="pagination">
                <div class="pagination-info">
                    Showing {{ $detailBarang->firstItem() ?? 0 }} to {{ $detailBarang->lastItem() ?? 0 }} of {{ $detailBarang->total() }} entries
                </div>
                <div class="pagination-buttons">
                    @if ($detailBarang->onFirstPage())
                        <span class="page-link disabled">Previous</span>
                    @else
                        <a class="page-link" href="{{ $detailBarang->previousPageUrl() }}">Previous</a>
                    @endif

                    @foreach ($detailBarang->getUrlRange(1, $detailBarang->lastPage()) as $page => $url)
                        @if ($page == $detailBarang->currentPage())
                    ...
                    @if ($detailBarang->hasMorePages())
                        <a class="page-link" href="{{ $detailBarang->nextPageUrl() }}">Next</a>
                    @else
