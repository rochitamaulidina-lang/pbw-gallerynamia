...
<head>
    ...
    <title>Tambah Detail Penjualan - Gallery Namia</title>
    ...
    <style>
        /* ========== CSS SAMA PERSIS DENGAN DETAIL BELI CREATE ========== */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        select.form-control-custom {
            ...
            background-repeat: no-repeat; background-position: right 1rem center; background-size: 0.9rem;
        }
        .select2-container--default .select2-selection--single {
            border: 2px solid #e2e8f0; border-radius: 14px; height: 38px; padding: 4px 8px;
        ...
        .select2-dropdown { border-radius: 14px; border-color: #e2e8f0; font-size: 0.8rem; }
        .btn-action {
        ...
        #stokInfo { margin-top: 5px; font-size: 0.7rem; }
    ...
    <div id="content" class="p-3 p-md-4">
        ...
        <div class="form-card">
            <div class="card-header-custom">
                <h5><i class="fas fa-plus-circle mr-2" style="color:#ff8c1a;"></i> Tambah Detail Penjualan</h5>
            </div>
            <div class="card-body-custom">
                ...
                <form action="{{ route('detailpenjualan.store', $penjualan->no_jual) }}" method="POST" id="detailForm">
                    @csrf
                    ...
                    <div class="form-group-custom">
                        ...
                        <select class="form-control-custom select2" name="no_barang" id="barang" required>
                            ...
                            <option value="{{ $b->no_barang }}"
                                ...
                                {{ $b->no_barang }} - {{ $b->nama_barang }}
                                (Stok: {{ number_format($b->stok_barang, 0, ',', '.') }} {{ $b->ukuran }})
                            </option>
                            @endforeach
                        </select>
                    </div>
                    ...
                    <div class="form-group-custom">
                        ...
                        <input type="number" name="qty_jual" id="qtyJual" class="form-control-custom" placeholder="Masukkan jumlah" required min="1">
                        <small class="form-text text-muted" id="stokInfo"></small>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-money-bill-wave"></i> HARGA</label>
                        <input type="text" id="hargaBarang" class="form-control-custom" readonly placeholder="Akan terisi otomatis">
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-chart-line"></i> SUBTOTAL</label>
                        <input type="text" id="subtotalJual" class="form-control-custom" readonly placeholder="Akan terisi otomatis">
                    </div>

                    <div class="info-badge">
                        <i class="fas fa-info-circle"></i> Subtotal akan dihitung otomatis (Qty × Harga)
                    </div>
...
<script>
    const dataBarang = <?php echo json_encode($barang); ?>;
    ...
    $(document).ready(function() {
        ...
        function updateInfoBarang() {
            var selected = $('#barang').find(':selected');
            var harga = selected.data('harga');
            var stok = selected.data('stok');
            var ukuran = selected.data('ukuran');

            if (harga) {
                $('#hargaBarang').val('Rp ' + parseInt(harga).toLocaleString('id-ID'));
                $('#stokInfo').html('<i class="fas fa-box"></i> Stok tersedia: ' + parseInt(stok).toLocaleString('id-ID') + ' ' + ukuran);
                $('#stokInfo').css('color', '#28a745');
            } else {
                $('#hargaBarang').val('');
                $('#stokInfo').html('');
            }
        ...
        function calculateSubtotal() {
            var harga = $('#barang').find(':selected').data('harga');
            var qty = parseInt($('#qtyJual').val());
            if (harga && qty > 0) {
        ...
        function validateQty() {
            var stok = $('#barang').find(':selected').data('stok');
            var qty = parseInt($('#qtyJual').val());
            if (qty > stok) {
                $('#stokInfo').html('<i class="fas fa-exclamation-triangle" style="color:#dc3545;"></i> Stok tidak mencukupi! Stok tersedia: ' + stok);
                $('#stokInfo').css('color', '#dc3545');
