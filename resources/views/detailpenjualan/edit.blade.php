...
<head>
    ...
    <title>Ubah Detail Penjualan - Gallery Namia</title>
    ...
    <style>
        /* ========== CSS SAMA PERSIS DENGAN DETAIL BELI EDIT ========== */
        * { margin: 0; padding: 0; box-sizing: border-box; }
        ...
        .form-group-custom label {
            font-weight: 600; color: #2c5a6e; margin-bottom: 0.3rem; font-size: 0.75rem; letter-spacing: 0.3px; display: block;
        }
        ...
        .btn-action {
            border-radius: 30px; padding: 6px 20px; font-weight: 600; font-size: 0.75rem;
            transition: all 0.2s; display: inline-flex; align-items: center; gap: 8px;
            margin: 0 4px; text-decoration: none; border: none;
        }
        .btn-save {
            background: linear-gradient(95deg, #ffb347, #ff8c1a); color: white; box-shadow: 0 4px 12px rgba(255, 140, 26, 0.3);
        }
        .btn-save:hover { transform: translateY(-2px); box-shadow: 0 8px 18px rgba(255, 140, 26, 0.35); color: white; }
        .btn-cancel {
            background: #dc3545; color: white;
        }
        .btn-cancel:hover { background: #c82333; transform: translateY(-2px); color: white; }
        .info-badge {
            background: #eef2fa; padding: 6px 12px; border-radius: 30px;
            display: inline-flex; align-items: center; gap: 6px; color: #1f5e7e;
            font-size: 0.7rem; margin-top: 0.8rem; margin-bottom: 0.8rem;
        }
        .navbar-nav .nav-link {
        ...
        .stok-info {
            margin-top: 8px; padding: 6px 10px; border-radius: 12px; font-size: 0.7rem;
            display: flex; align-items: center; gap: 8px;
        }
        .stok-info.success { background: #d4edda; color: #155724; border-left: 3px solid #28a745; }
        .stok-info.warning { background: #fff3cd; color: #856404; border-left: 3px solid #ffc107; }
        .stok-info.danger { background: #f8d7da; color: #721c24; border-left: 3px solid #dc3545; }
        .jumlah-input.valid { border-color: #28a745 !important; background-color: #f0fff4 !important; }
        .jumlah-input.invalid { border-color: #dc3545 !important; background-color: #fff5f5 !important; }
        .modal-custom .modal-content {
            border-radius: 20px; border: none; box-shadow: 0 20px 35px -10px rgba(0, 0, 0, 0.2); overflow: hidden;
        }
        ...
        .btn-confirm:hover { background: #c82333; transform: translateY(-1px); box-shadow: 0 8px 18px rgba(220, 53, 69, 0.4); }
        @media (max-width: 768px) {
            #sidebar { margin-left: -240px; }
            #sidebar.active { margin-left: 0; width: 240px; }
            #content { margin-left: 0; width: 100%; }
            .form-card .card-header-custom { flex-direction: column; align-items: flex-start; }
        }
    </style>
</head>
<body>
    ...
    <div id="content" class="p-3 p-md-4">
        ...
        <div class="form-card">
            <div class="card-header-custom">
                <h5><i class="fas fa-edit mr-2" style="color:#ff8c1a;"></i> Ubah Detail Penjualan</h5>
            </div>
            <div class="card-body-custom">
                ...
                <form action="{{ route('detailpenjualan.update', [$penjualan->no_jual, $detail->no_barang]) }}" method="POST" id="updateForm">
                    @csrf
                    @method('PUT')

                    <div class="form-group-custom">
                        <label><i class="fas fa-hashtag"></i> NO JUAL</label>
                        <input type="text" class="form-control-custom" value="{{ $penjualan->no_jual }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-calendar-alt"></i> TANGGAL</label>
                        <input type="date" class="form-control-custom" value="{{ $penjualan->tgl_jual }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-user"></i> PELANGGAN</label>
                        <input type="text" class="form-control-custom" value="{{ $penjualan->pelanggan->nama_pelanggan ?? '-' }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-box"></i> BARANG</label>
                        <input type="text" class="form-control-custom" value="{{ $detail->no_barang }} - {{ $detail->barang->nama_barang ?? '-' }}" readonly>
                        <input type="hidden" name="no_barang" value="{{ $detail->no_barang }}">
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-ruler"></i> UKURAN</label>
                        <input type="text" class="form-control-custom" value="{{ $detail->barang->ukuran ?? '-' }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-money-bill-wave"></i> HARGA</label>
                        <input type="text" id="hargaBarang" class="form-control-custom" value="Rp {{ number_format($detail->barang->harga_barang ?? 0, 0, ',', '.') }}" readonly>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-calculator"></i> QTY</label>
                        <input type="number" name="qty_jual" id="qtyJual" class="form-control-custom jumlah-input" value="{{ $detail->qty_jual }}" required min="1">
                        <div id="stokInfoContainer"></div>
                    </div>

                    <div class="form-group-custom">
                        <label><i class="fas fa-chart-line"></i> SUBTOTAL</label>
                        <input type="text" id="subtotalJual" class="form-control-custom" readonly value="Rp {{ number_format($detail->subtotal_jual, 0, ',', '.') }}">
                    </div>

                    <div class="info-badge">
                        <i class="fas fa-info-circle"></i> Subtotal akan dihitung ulang otomatis jika Qty diubah.
                    </div>

                    <div class="mt-3" style="display: flex; gap: 10px; justify-content: flex-end;">
                        <a href="{{ route('penjualan.show', $penjualan->no_jual) }}" class="btn btn-action btn-cancel">
                            <i class="fas fa-times"></i> Batal
                        </a>
                        <button type="submit" name="proses" class="btn btn-action btn-save" id="submitBtn">
...
<script>
    var stokSekarang = {{ $detail->barang->stok_barang ?? 0 }};
    var qtyLama = {{ $detail->qty_jual }};
    var ukuran = '{{ $detail->barang->ukuran ?? '' }}';
    var hargaBarang = {{ $detail->barang->harga_barang ?? 0 }};
    ...
    $(document).ready(function() {
        ...
        function updateStokInfo() {
            let qtyBaru = parseInt($('#qtyJual').val());
            let stokAkhir = stokSekarang - (qtyBaru - qtyLama);

            if (isNaN(qtyBaru) || qtyBaru <= 0) {
                $('#stokInfoContainer').html(`
                    <div class="stok-info warning">
                        <i class="fas fa-exclamation-triangle"></i>
                        <span>Qty harus lebih dari 0.</span>
                    </div>
                `);
                $('#qtyJual').removeClass('valid').addClass('invalid');
            ...
            if (stokAkhir < 0) {
                $('#stokInfoContainer').html(`
                    <div class="stok-info danger">
                        <i class="fas fa-times-circle"></i>
                        <span><strong>Stok tidak mencukupi!</strong> Stok setelah perubahan: ${stokAkhir.toLocaleString('id-ID')} ${ukuran}</span>
                    </div>
                `);
                $('#qtyJual').removeClass('valid').addClass('invalid');
                $('#submitBtn').prop('disabled', true);
            } else {
                let selisih = qtyBaru - qtyLama;
                let pesan = `Stok saat ini: ${stokSekarang.toLocaleString('id-ID')} ${ukuran} | `;
                if (selisih > 0) pesan += `Akan mengurangi stok: ${selisih.toLocaleString('id-ID')} ${ukuran} | `;
                else if (selisih < 0) pesan += `Akan menambah stok: ${Math.abs(selisih).toLocaleString('id-ID')} ${ukuran} | `;
                else pesan += `Tidak ada perubahan stok | `;
                pesan += `Stok setelah perubahan: ${stokAkhir.toLocaleString('id-ID')} ${ukuran}`;

                $('#stokInfoContainer').html(`
                    <div class="stok-info success">
                        <i class="fas fa-check-circle"></i>
                        <span>${pesan}</span>
                    </div>
                `);
                $('#qtyJual').removeClass('invalid').addClass('valid');
        ...
        function hitungSubtotal() {
            let qtyBaru = parseInt($('#qtyJual').val());
            if (!isNaN(qtyBaru) && qtyBaru > 0 && hargaBarang > 0) {
                let subtotal = qtyBaru * hargaBarang;
                $('#subtotalJual').val('Rp ' + subtotal.toLocaleString('id-ID'));
            } else {
                $('#subtotalJual').val('');
            }
        }

        $('#qtyJual').on('input', function() {
            updateStokInfo();
            hitungSubtotal();
        });
        ...
        $('#updateForm').on('submit', function(e) {
            let qtyBaru = parseInt($('#qtyJual').val());
            let stokAkhir = stokSekarang - (qtyBaru - qtyLama);
            if (qtyBaru <= 0 || stokAkhir < 0) {
                e.preventDefault();
                alert('Qty tidak valid atau stok tidak mencukupi!');
                return false;
            }
            return true;
        });
