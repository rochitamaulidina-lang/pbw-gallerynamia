{{-- resources/views/detailbeli/create.blade.php --}}
<!DOCTYPE html>
<html lang="id">
...
<head>
    ...
    <title>Tambah Detail Pembelian - Gallery Namia</title>
    ...
    <style>
        /* ========== CSS SAMA PERSIS DENGAN PHP NATIVE (pembeliandetail-tambah.php) ========== */
        * {
            margin: 0;
            padding: 0;
...
<body>
        ...
        <div id="content" class="p-3 p-md-4">
            ...
            <div class="form-card">
                <div class="card-header-custom">
                    <h5><i class="fas fa-plus-circle mr-2" style="color:#ff8c1a;"></i> Tambah Detail Pembelian</h5>
                </div>
                <div class="card-body-custom">
                    ...
                    <form action="{{ route('detailbeli.store', $pembelian->no_beli) }}" method="POST" id="detailForm">
                        @csrf
                        ...
                        <div class="form-group-custom">
                            ...
                            <select class="form-control-custom select2" name="no_bahan" id="bahanBaku" required>
                                <option selected disabled value="">--- Pilih Bahan Baku ---</option>
                                @foreach($bahanBaku as $b)
                                <option value="{{ $b->no_bahan }}"
                                    data-stok="{{ $b->stok_bahan }}"
                                    data-satuan="{{ $b->satuan }}"
                                    data-harga="{{ $b->harga_beli }}">
                                    {{ $b->no_bahan }} - {{ $b->nama_bahan }}
                                    (Stok: {{ number_format($b->stok_bahan, 0, ',', '.') }} {{ $b->satuan }})
                                </option>
                                @endforeach
                            </select>
                        </div>
                        ...
                        <div class="form-group-custom">
                            ...
                            <input type="number" name="qty_beli" id="qtyBeli" class="form-control-custom" placeholder="Masukkan jumlah pembelian" required min="1">
                        ...
                        <div class="form-group-custom">
                            ...
                            <input type="text" id="subtotalBeli" class="form-control-custom" readonly placeholder="Akan terisi otomatis">
                        </div>

                        <div class="info-badge">
                            <i class="fas fa-info-circle"></i> Subtotal pembelian akan dihitung otomatis (Qty Beli × Harga Beli)
                        </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            function updateInfoBahan() {
                var selected = $('#bahanBaku').find(':selected');
                ...
                if (harga) {
                    ...
                    $('#stokInfo').html('<i class="fas fa-database"></i> Stok saat ini: ' + parseInt(stok).toLocaleString('id-ID') + ' ' + satuan);
            ...
            function calculateSubtotal() {
                var harga = $('#bahanBaku').find(':selected').data('harga');
                var qty = parseInt($('#qtyBeli').val());
                if (harga && qty > 0) {
            ...
            $('#detailForm').on('submit', function(e) {
                ...
                if (isNaN(qty) || qty <= 0) {
                    e.preventDefault();
                    alert('Qty beli harus lebih dari 0!');
                    return false;
                }
                return true;
            });
